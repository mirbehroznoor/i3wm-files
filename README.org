...
For those who use the lightweight i3status, please take note the ~/i3/values~ folder acts as the config file and store the values for i3status to read. For example: brightness value is sent to ~/values/brightness/~ and from there, i3status reads the value for the bar. *I have configured it to the extent that I do not feel like using i3blocks anymore ;)*
